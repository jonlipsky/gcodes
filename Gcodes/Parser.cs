...
public class Parser
{
    ...
    public bool GetFinished()
    {
        ...
        for (var i = _index; i < _tokens.Count; i += 2)
        {
            if (_tokens[i].Kind != TokenKind.N)
                return false;
        }

        for (var j = _index + 1; j < _tokens.Count; j += 2)
        {
    ...
    {
        var got = ParseGCode() ?? ParseMCode() ?? ParseTCode() ?? (Code?)ParseOCode();
    ...
    private Token? ParseInteger()
    {
        ...
        if (numberTok.Value != null && (numberTok.Value.Contains('.') || numberTok.Value.Contains('-')))
        {
